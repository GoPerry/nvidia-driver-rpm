...
while [ "$1" ]; do
  ...
  -l, --latest
      ...
      most recent Linux-x86_64 driver version number.
  ...
  --glvnd-glx-client, --no-glvnd-glx-client
      By default, the NVIDIA OpenGL driver will be installed with
      the new GLVND architecture, to support coexisting with
      other GLVND-compliant OpenGL drivers. However, some
      applications which do not conform to the Linux OpenGL ABI
      may not be fully compatible with a GLVND-based OpenGL
      driver. The --no-glvnd-glx-client option will select a
      non-GLVND GLX client library (libGL.so.1), which may help
      to avoid compatibility issues with such applications.
