...
compile_test() {
    case "$1" in
      ...
      pci_dma_mapping_error)
        ...
        of_parse_phandle)
            #
            # Determine if the of_parse_phandle function is present.
            #
            CODE="
            #if defined(NV_LINUX_OF_H_PRESENT)
            #include <linux/of.h>
            #endif
            void conftest_of_parse_phandle() {
                of_parse_phandle();
            }"

            compile_check_conftest "$CODE" "NV_OF_PARSE_PHANDLE_PRESENT" "" "functions"
        ;;
        ...
        drm_helper_mode_fill_fb_struct_has_const_mode_cmd_arg)
            ...
            compile_check_conftest "$CODE" "NV_DRM_HELPER_MODE_FILL_FB_STRUCT_HAS_CONST_MODE_CMD_ARG" "" "types"
        ;;
