...
int __init nvidia_init_module(void)
{
    ...
    if (rc < 0)
    {
        nv_printf(NV_DBG_INFO, "NVRM: Ebridge init failed\n");
    }
...
)
{
    ...
    else
    {
        ...
        if ((status = rm_is_supported_device(sp, nv, &pmc_boot_0, &pmc_boot_42)) != NV_OK)
        {
...
)
{
     ...
     */
    status = rm_is_supported_device(sp, nv, &pmc_boot_0, &pmc_boot_42);
    if ((status != NV_OK) || (pmc_boot_0 == 0xFFFFFFFF))
    {
        nv_printf(NV_DBG_ERRORS,
